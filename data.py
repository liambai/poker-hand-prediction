...
c.execute('''CREATE TABLE Games(game_ID varchar(256) not null, stakes varchar(256), cards varchar(256), pre_flop_pot float, \
    post_flop_pot float, post_turn_pot float, post_river_pot float)''')
c.execute('''CREATE TABLE Hands(game_ID varchar(256) not null, player_ID varchar(256) not null, card_1 varchar(256), \
    card_2 varchar(256), bets float, collects float, chips_at_begining float)''')
